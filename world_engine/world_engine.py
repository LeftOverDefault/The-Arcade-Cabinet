...
class WorldEngine:
    ...
    def run(self):
        while self.running == True:
            ...
            # Sprite("./assets/sprite/mouse_img.png", ((((pygame.mouse.get_pos()[0] / (self.config.screen_multiplier / self.config.display_surface_multiplier)) + self.camera.offset.x) // self.config.tile_size) * self.config.tile_size, (((pygame.mouse.get_pos()[1] / (self.config.screen_multiplier / self.config.display_surface_multiplier)) + self.camera.offset.y) // self.config.tile_size) * self.config.tile_size), self.camera)
            self.camera.draw()
